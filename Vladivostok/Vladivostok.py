...
def least_sq_loggamma(x, c, loc, scale):
    return st.loggamma.pdf(x, c, loc, scale)
...
data = pd.read_csv("Vladivostok.csv", ';')
...
plt.show()
params_winter = st.norm.fit(winter)
params_spring = st.norm.fit(spring)
params_summer = st.norm.fit(summer)
params_autumn = st.loggamma.fit(autumn)
...
print(params_autumn)
print(params_z)
params_winter_lsq, pcovWinter = curve_fit(least_sq_norm, winter, st.norm.pdf(winter, *params_winter))
...
params_autumn_lsq, pcovAutumn = curve_fit(least_sq_loggamma, autumn, st.loggamma.pdf(autumn, *params_autumn))
...
qn_norm = st.norm.ppf(percs / 100.0, *params_winter)
...
qn_norm = st.loggamma.ppf(percs / 100.0, *params_autumn)
plt.figure(figsize=(10, 10))
plt.plot(qn_first, qn_norm, ls="", marker="o", markersize=6)
autumn_lin = np.linspace(autumn.min(), autumn.max())
plt.plot(autumn_lin, autumn_lin, color="k", ls="--")
plt.xlabel(f'Эмпирическое распределение')
plt.ylabel('Теоретическое (лог-гамма) распределение')
...
ks = st.kstest(winter, 'norm', params_winter)
print(ks)
...
ks = st.kstest(autumn, 'loggamma', params_autumn)
print(ks)

ks = st.kstest(z, 'gamma', params_z)
print(ks)

ks = st.kstest(winter, 'norm', params_winter_lsq)
print(ks)
...
ks = st.kstest(autumn, 'loggamma', params_autumn_lsq)
print(ks)
